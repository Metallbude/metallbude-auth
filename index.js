    ...
    for (const orderEdge of orders) {
      ...
      for (const returnEdge of returns) {
        ...
        returnRequests.push({
          ...
          customerEmail: '',
          requestDate: order.processedAt || order.createdAt || new Date().toISOString(), // 🔥 FIX: Use actual order dates since Return doesn't have createdAt
      ...
      try {
        ...
        for (const returnEdge of orderReturns) {
          ...
          const customerReturn = {
            ...
            requestDate: order.processedAt || new Date().toISOString(), // 🔥 FIX: Use actual order date since Return doesn't have createdAt
...
// ============================================================================
// AI IMAGE GENERATION (STABILITY AI - ROOM REDESIGN & MOODBOARD)
// ============================================================================

/**
 * POST /ai/redesign
 * Room redesign via Stability AI SD3 (image-to-image)
 * Body: { image: base64, style: string, custom_prompt?: string, strength: number (0-1) }
 */
app.post('/ai/redesign', async (req, res) => {
  try {
    if (!STABILITY_API_KEY) {
      return res.status(503).json({ error: 'Image AI not configured' });
    }

    const { image, style, custom_prompt, strength } = req.body || {};

    if (!image || !style) {
      return res.status(400).json({ error: 'Missing required fields: image, style' });
    }

    let strengthValue = typeof strength === 'number' ? strength : 0.6;
    if (Number.isNaN(strengthValue)) strengthValue = 0.6;
    strengthValue = Math.max(0, Math.min(1, strengthValue));

    const stylePrompts = {
      industrial: 'industrial style interior with exposed metal, raw steel, concrete walls, metal shelving',
      minimalist: 'minimalist clean design, neutral tones, simple forms, uncluttered space',
      scandinavian: 'scandinavian style, light wood, white walls, cozy textiles, hygge atmosphere',
      modern: 'modern contemporary design, sleek furniture, bold accents, open layout',
      rustic: 'rustic style with reclaimed wood, warm metals, natural textures, vintage accents',
      luxury: 'luxury interior, marble surfaces, gold accents, premium materials, elegant lighting',
    };

    const baseStyleKey = String(style || '').toLowerCase();
    const basePrompt = stylePrompts[baseStyleKey];
    if (!basePrompt) {
      return res.status(400).json({ error: 'Unsupported style', supportedStyles: Object.keys(stylePrompts) });
    }
    ...
    // Decode incoming base64 image (may be plain base64 or data URI)
    ...
    const headers = {
      ...
      ...(
        hasFormDataGetHeaders && typeof form.getHeaders === 'function'
          ? form.getHeaders()
          : {}
      ),
    };
    ...
    const response = await axios.post(
      ...
      form,
      {
        ...
        timeout: 60000, // 60s timeout for slower generations
      }
    );
    ...
    console.error('❌ /ai/redesign error:', error.response?.data || error.message || error);
    const status = error.response?.status || 500;
    return res.status(status >= 400 && status < 600 ? status : 500).json({
      error: 'Failed to generate redesigned image',
      details: error.response?.data || error.message,
    });
  }
});
...
/**
 * POST /ai/moodboard
 * Moodboard generation via Stability AI SD3 (text-to-image)
 * Body: { style: string, room_type?: string, color_palette?: string, additional_notes?: string }
 */
app.post('/ai/moodboard', async (req, res) => {
  try {
    if (!STABILITY_API_KEY) {
      return res.status(503).json({ error: 'Image AI not configured' });
    }

    const { style, room_type, color_palette, additional_notes } = req.body || {};

    if (!style) {
      return res.status(400).json({ error: 'Missing required field: style' });
    }

    const parts = [
      'Interior design moodboard, flat lay collage,',
      `${style} style`,
    ];

    if (room_type) parts.push(String(room_type));
    if (color_palette) parts.push(`${color_palette} color scheme`);

    parts.push('professional interior design presentation');

    if (additional_notes) parts.push(String(additional_notes));
    ...
    console.error('❌ /ai/moodboard error:', error.response?.data || error.message || error);
    const status = error.response?.status || 500;
    return res.status(status >= 400 && status < 600 ? status : 500).json({
...
// ============================================================================
// AI CHAT WITH PRODUCT RECOMMENDATIONS (CLAUDE + SHOPIFY RAG)
// ============================================================================

/**
 * POST /ai/chat
 * Body: { messages: [{ role: 'user'|'assistant', content: string }] }
 * Returns: { role: 'assistant', content, products: [...] }
 */
app.post('/ai/chat', async (req, res) => {
  try {
    if (!ANTHROPIC_API_KEY) {
      return res.status(503).json({ error: 'Chat AI not configured' });
    }
    ...
    // Very simple keyword extraction: take unique, longer words
    const keywordCandidates = userText
      .toLowerCase()
      .replace(/[^a-zA-Z0-9äöüßéèàùîôç\s]/g, ' ')
      .split(/\s+/)
      .filter(Boolean)
      .filter(w => w.length >= 3);

    const keywords = Array.from(new Set(keywordCandidates)).slice(0, 12);

    // Reuse Shopify admin products pattern (similar to /api/products)
    const PRODUCTS_QUERY = `
      query MetallbudeProducts($first: Int!) {
    ...
    const edges = productsResponse.data?.data?.products?.edges || [];
    const products = edges.map(edge => edge.node).filter(Boolean);
    ...
    const scoredProducts = products.map(p => {
      const textParts = [
        ...
        p.description || '',
        p.productType || '',
        ...(p.collections?.edges || []).map(e => e?.node?.title || ''),
      ].join(' ').toLowerCase();
      ...
      for (const kw of keywords) {
        if (textParts.includes(kw)) {
          score += 1;
          matchedKeywords.push(kw);
        }
      }

      // Slight boost for Metallbude-typical categories
      if (/table|tisch|shelf|regal|rack|halter|ladder|leiter|sofa|chair|sessel/.test(textParts)) {
        score += 0.5;
      }
    ...
    const productsContext = topMatches.map(({ product, matchedKeywords }) => {
      const priceAmount = product.priceRangeV2?.minVariantPrice?.amount;
      const priceCurrency = product.priceRangeV2?.minVariantPrice?.currencyCode || 'EUR';
      ...
      return {
        ...
        collections: (product.collections?.edges || []).map(e => e?.node?.title).filter(Boolean),
        matchedKeywords,
    ...
    const systemPrompt = `
You are the Metallbude AI design assistant. Metallbude sells minimalist metal furniture and home accessories.
You help customers find products, suggest styles, and answer design questions.
When recommending products, always reference specific Metallbude products by name.
Be friendly, concise, and helpful.
Respond in the same language the user writes in (German or English).

Here is structured context about potentially relevant Metallbude products:
${JSON.stringify(productsContext, null, 2)}

When you recommend products, explain briefly *why* each product fits the user's needs.
    `.trim();
...
Relevant Metallbude products (JSON):
${JSON.stringify(productsContext, null, 2)}

Please answer the user. Then, for each recommended product, create a short reason.
Respond in a concise style.
        `.trim(),
      },
    ];
    ...
    let assistantText = '';
    try {
      const contentBlocks = anthropicResponse.data?.content || [];
      assistantText = contentBlocks
        .map(block => (typeof block?.text === 'string' ? block.text : block?.content || ''))
        .join('\n')
        .trim();
      if (!assistantText && typeof anthropicResponse.data?.content === 'string') {
        assistantText = anthropicResponse.data.content;
      }
    } catch (e) {
      console.error('⚠️ Failed to parse Anthropic response text:', e?.message || e);
      assistantText = anthropicResponse.data?.content?.toString?.() || '';
    }

    const mappedProducts = productsContext.map(p => {
      const priceNumber = p.price != null ? Number(p.price) : null;
      let reason = '';

      if (assistantText) {
        const lower = assistantText.toLowerCase();
        if (p.title && lower.includes(p.title.toLowerCase())) {
          reason = `Empfohlen, weil es gut zu deiner Beschreibung passt (${(p.matchedKeywords || []).join(', ')})`;
        }
      }
    ...
    return res.json({
      role: 'assistant',
      content: assistantText || 'Ich konnte deine Frage beantworten, habe aber keine zusätzlichen Details vom KI-Modell erhalten.',
...
app.post('/zendesk/webhook/chat-reply', async (req, res) => {
  try {
    ...
    // Skip if the customer themselves sent the message (we only push for agent/bot replies)
    // Since we removed the Zendesk trigger condition "Aktueller Benutzer ist nicht Endbenutzer"
    // (it doesn't work reliably for Messaging), we filter server-side instead.
    const senderName = (current_user_name || '').trim();
    const requesterNameNorm = (requester_name || '').trim();
    if (senderName && requesterNameNorm && senderName.toLowerCase() === requesterNameNorm.toLowerCase()) {
      console.log(`⏭️ Skipping customer's own message for ticket #${ticket_id} (sender: "${senderName}" = requester)`);
      return res.json({ received: true, skipped: 'customer message' });
    }

    // Skip empty system/bot messages (no content)
    const senderLower = senderName.toLowerCase();
    const isSystemSender = senderLower === 'system' || senderLower === 'chat bot' || senderLower === '' || senderLower === 'answer bot';
