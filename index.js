...
// 🔥 DEMO: Generate demo returns using real order dates for testing
async function generateDemoReturns(customerEmail) {
  try {
    console.log('🧪 Generating demo returns with real order dates...');
    
    // Get real customer orders first to use their actual dates
    let realOrders = [];
    ...
    if (config.adminToken) {
      try {
        ...
        if (response.data?.data?.customers?.edges?.[0]?.node?.orders?.edges) {
          realOrders = response.data.data.customers.edges[0].node.orders.edges.map(e => e.node);
          console.log(`🧪 Found ${realOrders.length} real orders to base demo returns on`);
        }
      } catch (error) {
        console.log('⚠️ Could not fetch real orders for demo data, using fallback dates');
      }
    }

    // Demo return reasons and statuses
    const demoReasons = ['defective', 'wrong_item', 'not_as_described', 'color_finish', 'size_dimensions', 'quality_material', 'changed_mind'];
    const demoStatuses = ['requested', 'open', 'approved', 'rejected', 'closed', 'declined', 'completed'];
    
    const demoReturns = [];
    const maxDemoReturns = Math.min(14, realOrders.length || 8); // Limit to real orders or 8 max
    
    for (let i = 0; i < maxDemoReturns; i++) {
      const realOrder = realOrders[i];
      const orderNumber = realOrder ? realOrder.name.replace('#', '') : `13${6152 - i}`;
      
      // Use real order date or create realistic past dates
      let orderDate;
      if (realOrder && realOrder.processedAt) {
        orderDate = realOrder.processedAt;
      } else if (realOrder && realOrder.createdAt) {
        orderDate = realOrder.createdAt;
      } else {
        // Create realistic dates spread over past months instead of all today
        const daysAgo = Math.floor(Math.random() * 180) + 7; // 7 to 187 days ago
        const hoursOffset = Math.floor(Math.random() * 24);
        const minutesOffset = Math.floor(Math.random() * 60);
        orderDate = new Date(Date.now() - (daysAgo * 24 * 60 * 60 * 1000) - (hoursOffset * 60 * 60 * 1000) - (minutesOffset * 60 * 1000)).toISOString();
      }
      
      // Create demo items based on real order items if available
      const items = [];
      if (realOrder && realOrder.lineItems && realOrder.lineItems.edges.length > 0) {
        // Use real order items
        for (const itemEdge of realOrder.lineItems.edges) {
          const item = itemEdge.node;
          items.push({
            lineItemId: item.id,
            productId: item.variant?.id || `demo-product-${i}`,
            title: item.title,
            imageUrl: item.variant?.image?.url || null,
            quantity: item.quantity,
            price: parseFloat(item.variant?.price || '29.99'),
            sku: item.variant?.sku || `SKU-${i}`,
            variantTitle: item.variant?.title || 'Standard',
            returnReason: demoReasons[Math.floor(Math.random() * demoReasons.length)],
            customerNote: 'Demo return for testing UI'
          });
        }
      } else {
        // Create demo items
        ...
        for (let j = 0; j < itemCount; j++) {
          items.push({
            lineItemId: `demo-item-${i}-${j}`,
            productId: `demo-product-${i}-${j}`,
            title: `Test Product ${i + 1}${itemCount > 1 ? ` - Item ${j + 1}` : ''}`,
            imageUrl: null,
            quantity: Math.floor(Math.random() * 2) + 1,
            price: Math.floor(Math.random() * 100) + 20,
            sku: `TEST-SKU-${i}-${j}`,
            variantTitle: 'Demo Variant',
            returnReason: demoReasons[Math.floor(Math.random() * demoReasons.length)],
            customerNote: 'Demo return for testing'
          });
        }
      }
      ...
      demoReturns.push({
        id: `demo-return-${i}`,
        orderId: realOrder ? realOrder.id : `demo-order-${orderNumber}`,
        orderNumber: orderNumber,
        items: items,
        reason: items[0].returnReason,
        additionalNotes: `Demo return ${i + 1} for UI testing`,
        preferredResolution: Math.random() > 0.5 ? 'refund' : 'exchange',
        customerEmail: customerEmail,
        requestDate: orderDate, // 🔥 USE REAL ORDER DATE!
        status: demoStatuses[Math.floor(Math.random() * demoStatuses.length)],
        shopifyReturnRequestId: `demo-shopify-return-${i}`,
      });
    }
    
    console.log(`🧪 Generated ${demoReturns.length} demo returns with real dates:`);
    demoReturns.forEach(ret => {
      console.log(`  - Order #${ret.orderNumber}: ${ret.status} (${ret.requestDate})`);
    });
    
    return demoReturns;
    
  } catch (error) {
    console.error('❌ Error generating demo returns:', error);
    return [];
  }
}
...
app.get('/returns', authenticateAppToken, async (req, res) => {
  try {
    ...
    // 🔥 DEMO: Generate demo returns if none exist (for testing UI)
    if (shopifyReturns.length === 0) {
      console.log('🧪 No real returns found, generating demo data for UI testing...');
      
      shopifyReturns = await generateDemoReturns(customerEmail);
      console.log(`🧪 Generated ${shopifyReturns.length} demo returns for testing`);
    }
